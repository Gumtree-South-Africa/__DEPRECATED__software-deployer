...
class AuroraGeneratorTest(unittest.TestCase):
    ...
    def containsPipelineNotifyAndDeployMonitorNotify(self, tasklist):
        #self.log.info("lem: %s" % self.flatten(map(lambda i: i['tasks'], tasklist['stages'])))
        stages_exists = ('Deploy monitor notify deploying' in map(lambda i: i['name'], tasklist['stages'])
        and

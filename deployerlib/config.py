...
class Config(AttrDict):
    ...
    def get_lb_for_host(self, hostname):
        ...
        if not host_dc:
            self.log.warn('Host {0} not found in configuration'.format(hostname))
            return None, None, None

        if not 'loadbalancers' in self[host_dc]:
            self.log.warn('Host {0} is in {1}, but there are no loadbalancers for this datacentre'.format(
              hostname, host_dc))
            return None, None, None
        ...
        if len(self[host_dc]['loadbalancers'].keys()) > 1:
            self.log.warn('More than one load balancer found for {0}, using the first one ({1})'.format(
              hostname, lb))
